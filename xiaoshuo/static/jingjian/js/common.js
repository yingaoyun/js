//搜索框
function search(){const placestr = '可搜书名和作者，请您少字也别输错字。';document.write('<form name="t_frmsearch" method="post" action="/search/"  target="_blank" class="search-form" onsubmit="return chkval()"><input autocomplete="off" style="width: 260px;" id="searchkey" type="text" name="searchkey" class="input-text input-key" placeholder="'+placestr+'"><button type="submit" style="padding: 0 0 1px;width: 15%;" name="Submit" id="search_btn" class="btn-tosearch" value="搜索" title="搜索" >搜索</button></form>');}
function chkval(){
	let skey = '';
	if($('#searchkey').val() == ''){
