		...
		if($("#downul")[0].scrollHeight>305)
		{
			...
			$(this).removeClass('zk').addClass('ss').text('收缩部分');
				...
				e.preventDefault(); 
       				...
       				//影片演员页收缩展开
		if($("#sctorlist").length > 0)
	{
		if($("#sctorlist")[0].scrollHeight>360)
		{
			$("#downzk").show();
			$("#sctorlist").height(353);
			$("#downzk").click(function(e){
			if($(this).hasClass('ss')){
			$(this).removeClass('ss').addClass('zk').text('展开全部');
		}else{
			$(this).removeClass('zk').addClass('ss').text('隐藏部分');
		}						
				if($("#sctorlist").height()>360)
				{
					var h = $("#sctorlist")[0].scrollHeight;
					$("#sctorlist").height(353);
...
function fn(likey,tag){
	if (tf[tag]){
		if (h[tag]>=minheight){
			document.getElementById("main").style.height = "28px";
		...
		if (h[tag]<=maxheight){
			document.getElementById("main").style.height = "56px";
			setTimeout(function(){fn(likey,tag)},1);
			h[tag] += 10;
		} else {
			likey.innerHTML = "收起";
